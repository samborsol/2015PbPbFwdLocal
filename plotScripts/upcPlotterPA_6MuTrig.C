...
void upcPlotterPA_6MuTrig(){
	...
	TCut jpsiMass = "Di_mass<3.2 && Di_mass>3.0";
	...
	t->Draw("Di_pt >> h1",trig1+jpsiMass+rapCut);
	t->Draw("Di_pt >> h2",trig2+jpsiMass+rapCut);
	t->Draw("Di_pt >> h3",trig3+jpsiMass+rapCut);
	t->Draw("Di_pt >> h4",trig4+jpsiMass+rapCut);
	t->Draw("Di_pt >> h5",trig5+jpsiMass+rapCut);
	t->Draw("Di_pt >> h6",trig6+jpsiMass+rapCut);

	h1->SetTitle("Dimuon Pt (J/Psi)");
...
//	h1->SetMaximum(10000);
	...
	h6->SetLineWidth(2); 
        ...
        TLegend *leg = new TLegend(0.1,0.70,0.95,0.9);
	leg->SetFillStyle(0);
