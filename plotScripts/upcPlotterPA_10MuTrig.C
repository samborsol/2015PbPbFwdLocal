...
*/
	...
	t->Draw("Di_rapidity >> h1",trig1+massCut+ptCut);
	t->Draw("Di_rapidity >> h2",trig2+massCut+ptCut);
	t->Draw("Di_rapidity >> h3",trig3+massCut+ptCut);
	t->Draw("Di_rapidity >> h4",trig4+massCut+ptCut);
	t->Draw("Di_rapidity >> h5",trig5+massCut+ptCut);
	t->Draw("Di_rapidity >> h6",trig6+massCut+ptCut);

	TCanvas *c1 = new TCanvas("c1","hists with different scales",700,700);
	gStyle->SetOptStat(kFALSE);

	h1->SetTitle("Dimuon Rapidity (J/Psi)");
