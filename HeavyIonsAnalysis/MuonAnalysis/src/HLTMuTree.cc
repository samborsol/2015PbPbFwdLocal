...
HLTMuTree::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	if (doReco) {
		...
		for(unsigned int i=0; i<muons->size(); i++){
			edm::RefToBase<reco::Muon> muCand(muons,i);
			if (muCand.isNull()) continue;
			if (muCand->innerTrack().isNonnull()) {
				if (muCand->isTrackerMuon() && fabs(muCand->innerTrack()->eta()) < 10) {  //was 2.4
					for (unsigned int j=i+1; j<muons->size(); j++){
						edm::RefToBase<reco::Muon> muCand2(muons,j);
						if (muCand2.isNull()) continue;
						if (muCand2->innerTrack().isNonnull()) {
						      if (muCand2->isTrackerMuon() && fabs(muCand2->innerTrack()->eta()) < 10) {
                                                                vector<TransientTrack> t_tks;
								t_tks.push_back(theTTBuilder->build(*muCand->track()));  // pass ent)
								t_tks.push_back(theTTBuilder->build(*muCand2->track())); // other refs inside.S?
								...
								if (myVertex.isValid()) {
									...
									DiMu.pt1[nDiMu] = trk->pt();
									DiMu.eta1[nDiMu] = trk->eta();
									DiMu.phi1[nDiMu] = trk->phi();
									DiMu.dxy1[nDiMu] = trk->dxy(vertex->begin()->position());
									DiMu.dz1[nDiMu] = trk->dz(vertex->begin()->position());
									DiMu.charge1[nDiMu] = trk->charge();
									DiMu.pt2[nDiMu] = trk2->pt();
									DiMu.eta2[nDiMu] = trk2->eta();
									DiMu.phi2[nDiMu] = trk2->phi();
									DiMu.dxy2[nDiMu] = trk2->dxy(vertex->begin()->position());
									DiMu.dz2[nDiMu] = trk2->dz(vertex->begin()->position());
									DiMu.charge2[nDiMu] = trk2->charge();
									DiMu.charge[nDiMu] = trk->charge() + trk2->charge();

									DiMu.nTrkHit1[nDiMu] = trk->hitPattern().numberOfValidTrackerHits();
									DiMu.nTrkHit2[nDiMu] = trk2->hitPattern().numberOfValidTrackerHits();
									DiMu.nMuHit1[nDiMu] = trk->hitPattern().numberOfValidMuonHits();
									DiMu.nMuHit2[nDiMu] = trk2->hitPattern().numberOfValidMuonHits();
									DiMu.nTrkLayers1[nDiMu] = trk->hitPattern().trackerLayersWithMeasurement();
									DiMu.nTrkLayers2[nDiMu] = trk2->hitPattern().trackerLayersWithMeasurement();
